...
const Banner = ({banner}) => {
  ...
  return (
    ...
    <div onClick={goToDetails} className="h-full relative w-[clamp(8rem,10vw,16rem)] border pb-2 hover:border-green-400 hover:bg-[#333333] cursor-pointer hover:scale-105 duration-75 flex flex-col shrink-0 bg-[#1e1e1e] rounded-xl">
      <Image width={200} height={400} className="h-[85%] w-full rounded-tr-xl rounded-tl-xl" src={`https://image.tmdb.org/t/p/original/${banner.poster_path || null}`} alt="no poster available" />
        <p className="text-white w-35 overflow-clip text-nowrap text-ellipsis ml-2 font-medium text-[clamp(0.8rem,0.8vw,2rem)]" >{banner.name || banner.title || banner.original_title}</p>
